...
static rotary_encoder_err_t
rotary_encoder_device_initialize(rotary_encoder_t const *encoder) {
  return encoder->interface.device_initialize
...
static rotary_encoder_err_t
rotary_encoder_device_deinitialize(rotary_encoder_t const *encoder) {
  return encoder->interface.device_deinitialize
...
static rotary_encoder_err_t
rotary_encoder_device_get_step_count(rotary_encoder_t const *encoder,
                                     int64_t *step_count) {
  return encoder->interface.device_get_step_count
             ? encoder->interface.device_get_step_count(
                   encoder->interface.device_user, step_count)
             : ROTARY_ENCODER_ERR_NULL;
}
...
rotary_encoder_position_to_step_count(rotary_encoder_t const *encoder,
                                      float32_t position) {
  assert(encoder->config.step_change > 0.0F);

  float32_t step_count =
      rotary_encoder_wrap_position(position) / encoder->config.step_change;

  return (int64_t)roundf(step_count);
}

static inline float32_t
rotary_encoder_step_count_to_position(rotary_encoder_t const *encoder,
                                      int64_t step_count) {
  assert(encoder->config.step_change > 0.0F);
...
rotary_encoder_initialize(rotary_encoder_t *encoder,
